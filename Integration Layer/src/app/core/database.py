...
Tests override :func:`get_db` to inject a transactional session that rolls
back per test (see ``tests/conftest.py``).
"""
...
class Base(DeclarativeBase):
    """Declarative base for every ORM model in the Integration Layer.
...
class TimestampMixin:
    ...
    Shared by every table — the timestamps are infrastructure, not domain
    state, so a mixin is the right DRY boundary here. ``updated_at`` is set
    server-side on every UPDATE via ``onupdate=func.now()``; mutating
    services that hand the row to Pydantic afterwards must ``refresh()`` it
    (the same server-onupdate refresh trap the Backend documents).
    """
...
def get_engine() -> AsyncEngine:
    ...
    ``pool_pre_ping`` is on so connections dropped by the DB (restart, idle
    timeout) are detected and recycled transparently instead of surfacing as
    request failures.
    """
...
async def get_db() -> AsyncIterator[AsyncSession]:
    ...
    exception the active transaction is rolled back explicitly so a failed
    request never leaks partial state back into the connection pool.
    """
