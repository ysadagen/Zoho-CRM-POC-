...
``request_id`` bound by :class:`RequestIDMiddleware` is merged in
automatically via structlog contextvars.

This middleware must run *inside* :class:`RequestIDMiddleware` so the
request id is already in the context when the line is emitted. See
``main.py`` for the registration order.
"""
