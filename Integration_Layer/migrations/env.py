...
Reads only the database URL from the environment (via a minimal settings class)
and uses :data:`app.models.Base.metadata` as the autogenerate target. Runs in
...
``app.core.config``. The full application settings require secrets (e.g.
``ZOHO_TOKEN_ENCRYPTION_KEY``) that are irrelevant at migration time. A
minimal ``_MigrationSettings`` class reads only what Alembic needs so that
``alembic upgrade head`` works without a fully-populated ``.env``.
"""
