...
|---|---|---|
| D-1 | ~~Intelligence lives **inside the Backend** as new modules~~ → **REVERSED (2026-06-18, owner directive): Intelligence is a separate `Intelligence/` FastAPI service** sharing `inventory_db` | Original rationale (avoid a 4th service at POC scale) was overridden by the owner's requirement for service isolation. The separate service reads the Backend's CRM tables and **owns** the scoring tables via its own Alembic chain (`version_table=alembic_version_intelligence`); it validates the Backend's JWTs (shared secret). Consequence: lead scoring is **live-on-read + recompute** (the Backend no longer scores on write — see D-9 note), and the service defines its own read-models for the CRM tables (no Backend import). 2A data-foundation stays in the Backend. |
...
| 2B.4 Beat planning engine | ✅ Done | 2026-06-18 | migration `e4a6c8b2d5f1`; pure `compute_beat_plan` (§7) — BP-1 + visit-gap band edges + NULL-district LDS 0 + cluster-opportunity threshold + max_visits truncation + priority bands + zero-revenue cohort; `customer_metrics.handled_customers`; live `GET /intelligence/beat-plan?rep_user_id=&max_visits=` (clusters + suggested beat + ranked list); `visit_priority_scores` snapshot model + `visit_priority` enum. 11 new tests (89 scoring total); ruff+mypy clean |
| 2B.5 Recompute + snapshots | ✅ Done | 2026-06-18 | `recompute_service` + admin `POST /intelligence/recompute` (`{engine}` null = all); per-engine snapshot sweeps (lead re-score reconciles cohort drift; health/effort/beat snapshots); returns per-engine counts + `duration_ms`; config-versioning verified (snapshot carries new `config_id` after activation); nightly invocation documented in Backend README. 4 new tests; ruff+mypy clean |
| **Gate 2B — user testing** | 🟢 Ready | 2026-06-18 | All 2B slices green (93 scoring tests; full suite green). Four engines computable + snapshottable over seed data; canonical vectors LS-1/EE-1/CH-1/BP-1 hand-verified. Awaiting user review of scores via `/docs` before Phase 2C. |
